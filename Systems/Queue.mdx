...
You must follow the one-step setup. Setting up for customization is not required, but it might be useful if you're customizing certain parts.
...
The model contains a ModuleScript, a folder of queues that work by default, a GUI, and more. You must keep everything besides the queues that work by default. You should do this if you have a scripter that can implement custom queues via the Module. See [Deleting the ready-made queue](https://player.mintlify.app/Systems/Queue#deleting-the-ready-made-queue).
...
The UI elements are located under the GUI 'Queue". You can customize how the GUI `Queue` looks as much as you'd like. Some properties should not be edited to keep the queue system working.
...
<Check>
  If you're creating a custom queue via the `Setup` function, as long as you pass the start part and the join part, you can change things like Parents and Names.
</Check>
...
<Accordion title="Do not change:" defaultOpen icon="x" iconType="solid">
  ...
  - CanCollide (Must be off)
    ...
    <Warning>
      Changing these properties will cause the queues to not accept players without an error message!
    </Warning>
</Accordion>
...
local customization = {maxplayers, studstomove} -- since cooldowntime is omitted, it will be the default value
...
<ResponseField name="queue" type="string">
  The name of the queue the player has moved in. Can be `nil` if the player is not  in a queue, guaranteeing that `position` is also `nil`.
</ResponseField>
...
<ResponseField name="position" type="number">
  The new position of the player. Can be `nil` if the player is no longer in the queue.
</ResponseField>
